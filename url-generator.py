...
# from os import path
# import sys
# sys.path.append(path.abspath('../the-url-god-2'))
...
def upload_action(projectname, event=None):
    ...
    # get the tab 2 Domains
    domains = data['Domains'].values.tolist()
    for d in domains:
        currendirectory = os.getcwd()
        conn = sqlite3.connect(currendirectory+"\\db\\urldatabase.db")
        cur = conn.cursor()
        cur.execute("INSERT INTO domains VALUES (NULL,?,?)",
                    (projectname, d[0]))
        conn.commit()
        conn.close()
    progress_bar['value'] += 14
    root.update_idletasks()
    # get tab 3 Google location Servers
    ...
    for gv in googlevs:
        ...
        root.update_idletasks()
    # get tab 4 Source
    sources = data['Sources'].values.tolist()
    for s in sources:
        currendirectory = os.getcwd()
        conn = sqlite3.connect(currendirectory+"\\db\\urldatabase.db")
        cur = conn.cursor()
        cur.execute("INSERT INTO source VALUES (NULL,?,?)",
                    (projectname,  s[0].replace(" ", "%20")))
        conn.commit()
        conn.close()
    progress_bar['value'] += 14
    root.update_idletasks()
    # get tab 5 Medium
    medium = data['Medium'].values.tolist()
    for m in medium:
        currendirectory = os.getcwd()
        conn = sqlite3.connect(currendirectory+"\\db\\urldatabase.db")
        cur = conn.cursor()
        cur.execute("INSERT INTO medium VALUES (NULL,?,?)",
                    (projectname, m[0]))
        conn.commit()
        conn.close()
    progress_bar['value'] += 14
    root.update_idletasks()
    # get tab 6 Campaign
    campaing = data['Campaign'].values.tolist()
    for cam in campaing:
        currendirectory = os.getcwd()
        conn = sqlite3.connect(currendirectory+"\\db\\urldatabase.db")
        cur = conn.cursor()
        cam[0].replace(" ", "%20")
        cur.execute("INSERT INTO campaign VALUES (NULL,?,?)",
                    (projectname, cam[0].replace(" ", "%20")))
        conn.commit()
        conn.close()
    ...
    for l in languages:
        ...
        root.update_idletasks()
    # get tab variants save column in databse
    # sa
    v_sa = data['Variants']['sa'].values.tolist()
    for sa in v_sa:
        currendirectory = os.getcwd()
        conn = sqlite3.connect(currendirectory+"\\db\\urldatabase.db")
        cur = conn.cursor()
        cur.execute("INSERT INTO sa VALUES (NULL,?,?)",
                    (projectname, sa))
        conn.commit()
        conn.close()
        print(sa)
        root.update_idletasks()
    # rct
    v_rct = data['Variants']['rct'].values.tolist()
    for rct in v_rct:
        currendirectory = os.getcwd()
        conn = sqlite3.connect(currendirectory+"\\db\\urldatabase.db")
        cur = conn.cursor()
        cur.execute("INSERT INTO rct VALUES (NULL,?,?)",
                    (projectname, rct))
        conn.commit()
        conn.close()
        print(rct)
        root.update_idletasks()
    # esrc
    v_esrc = data['Variants']['esrc'].values.tolist()
    for esrc in v_esrc:
        currendirectory = os.getcwd()
        conn = sqlite3.connect(currendirectory+"\\db\\urldatabase.db")
        cur = conn.cursor()
        cur.execute("INSERT INTO esrc VALUES (NULL,?,?)",
                    (projectname, esrc))
        conn.commit()
        conn.close()
        print(esrc)
        root.update_idletasks()
    # cad
    v_cad = data['Variants']['cad'].values.tolist()
    for cad in v_cad:
        currendirectory = os.getcwd()
        conn = sqlite3.connect(currendirectory+"\\db\\urldatabase.db")
        cur = conn.cursor()
        cur.execute("INSERT INTO cad VALUES (NULL,?,?)",
                    (projectname, cad))
        conn.commit()
        conn.close()
        print(cad)
        root.update_idletasks()
    # uact
    v_uact = data['Variants']['uact'].values.tolist()
    for uact in v_uact:
        currendirectory = os.getcwd()
        conn = sqlite3.connect(currendirectory+"\\db\\urldatabase.db")
        cur = conn.cursor()
        cur.execute("INSERT INTO uact VALUES (NULL,?,?)",
                    (projectname, uact))
        conn.commit()
        conn.close()
        print(uact)
        root.update_idletasks()
    # ved
    v_ved = data['Variants']['sa'].values.tolist()
    for ved in v_ved:
        currendirectory = os.getcwd()
        conn = sqlite3.connect(currendirectory+"\\db\\urldatabase.db")
        cur = conn.cursor()
        cur.execute("INSERT INTO ved VALUES (NULL,?,?)",
                    (projectname, ved))
        conn.commit()
        conn.close()
        print(ved)
...
# TODO: cambiar variables segun las convenciones de Python-- SI funciona no lo toque huevon!!
...
def the_url_god_2():
    import theurlgod2.urlgenerator 

def thgeo():
    import thGeo.generate_urls
    import thGeo.front 
def thgeo_rss():
    import thGeo.front_rss
...
def export_csv():
    ...
    if select_project == "Select One Project":
        ...
        with open(currendirectory+'\\export\\urls.txt', 'a', encoding='utf8', newline='') as file:
            for l in urls:
                file.write(l[2]+"\n")
...
the_url = Button(tab2, text="The Url God 2",
                       command=the_url_god_2, bg="blue", fg='white', width=41, height=3)
the_url.grid(column=1, row=7)
THGEO = Button(tab2, text="THGEO",
                       command=thgeo, bg="blue", fg='white', width=41, height=3)
THGEO.grid(column=1, row=8)

THGEO_RSS = Button(tab2, text="THGEO RSS",
                       command=thgeo_rss, bg="blue", fg='white', width=41, height=3)
THGEO_RSS.grid(column=1, row=9)
